...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        newmask = cv2.bitwise_or(mask1, mask2)
        # Shows the detected colour from the mask
        res = cv2.bitwise_and(img, img, mask=newmask)

        # Annotate the colour detections
        # replace the '_' parameter with the appropiate variable
        contours, _ = cv2.findContours(newmask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        # ============
        # ↑ BOOTCAMPERS MODIFY ABOVE THIS COMMENT ↑
        # ============
        ...
        return newmask if return_mask else None
